...
function abrirPrediccionRiesgo(pacienteId) {
  ...
  authFetch('/api/ml/predecir/', {
    ...
    .then((data) => {
      ...
      document.getElementById('prediccion-nivel').textContent = data.riesgo_predicho || '—';

      const prob = data.probabilidad !== undefined && data.probabilidad !== null ? data.probabilidad : null;
      document.getElementById('prediccion-probabilidad').textContent = prob !== null ? `${prob}` : '—';

      document.getElementById('prediccion-puntuacion-clinica').textContent = data.puntuacion_clinica ?? '—';
      ...
      factoresUl.innerHTML = factores.length
        ? factores.map((f) => `
            <li class="list-group-item d-flex align-items-start justify-content-between gap-3">
              <div>
                <div style="font-weight:700;">${f.factor || ''}</div>
                <div style="font-size:12px;color:var(--text-muted);">${f.descripcion || ''}</div>
              </div>
              <span class="badge text-bg-light border" style="white-space:nowrap;">${f.impacto || ''}</span>
            </li>
          `).join('')
        : `<li class="list-group-item text-center" style="color:var(--text-muted);">Sin factores</li>`;
      ...
      recsUl.innerHTML = recs.length
        ? recs.map((r) => `
            <li class="list-group-item" style="font-size:13px;">${r}</li>
          `).join('')
        : `<li class="list-group-item text-center" style="color:var(--text-muted);">Sin recomendaciones</li>`;
      ...
      if (data.prediccion_modelo) {
        ...
        document.getElementById('prediccion-modelo-riesgo').textContent = data.prediccion_modelo.riesgo_predicho || '—';
        document.getElementById('prediccion-modelo-prob').textContent = data.prediccion_modelo.probabilidad ?? '—';
        ...
        if (dist && typeof dist === 'object') {
          ...
          body.innerHTML = Object.entries(dist)
            ...
            .map(([k, v]) => `
              <span class="badge border" style="background:rgba(26,107,205,.07);border-color:rgba(26,107,205,.2);color:var(--blue);font-weight:700;">
